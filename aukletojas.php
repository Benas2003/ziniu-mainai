<?php
  ...
  // Duomenų apie auklėtoją ir mokinį perdavimas
  session_start();
  $gavejas=$_SESSION['aukID'];
  $vardas=$_SESSION['aisV'];
  $pavarde=$_SESSION['aisP'];

  $conn = new mysqli($servername, $username, $password, $dbname);
  $sql = "SELECT ID, Pastas FROM Mokytojai WHERE ID='$gavejas'";
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        header("Location: Index.php?status=success?confirm=fail");
      $gavejasEL=$row['Pastas'];
   }
  } else {
   echo "0 results";
  }
  ...
  // El. pašto sukūrimas ir išsiuntimas
    ...
    /* Add a recipient. */
    $mail->addAddress($gavejasEL);

    /* Set the subject. */
    $mail->Subject = 'Pranešimas apie socialinių valandų skyrimą';
                                  ...
                                  <table border='0' cellpadding='0' cellspacing='0' width='100%'>
                                      <tr>
                                          <td style='color: #153643; font-family: Arial, sans-serif; font-size: 15px;'>
                                              <b>Mokiniui/-ei $vardas $pavarde priklauso 1 socialinė valanda už nesuprantamos temos paaiškinimą UKG mokiniui/-ei.</b>
                                          </td>
                                      </tr>
                                      <tr>
                                          <td>
    ...
    header("Location: Index.php?status=success?confirm=success");
?>
