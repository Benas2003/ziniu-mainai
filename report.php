<?php 
    ...
    $sql = "SELECT ID, Vardas, Pavarde, Klase, Pastas FROM Mokytojai WHERE Statusas='Aktyvi'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        $skaiciusM++;
        $mokytojoID[$skaiciusM]=$row["ID"];
        $vardas[$skaiciusM]=$row["Vardas"];
        $pavarde[$skaiciusM]=$row["Pavarde"];
        $klasesNumeris[$skaiciusM]=$row["Klase"];
        $pastas[$skaiciusM]=$row["Pastas"];
    }
    } else {
    echo "0 results";
    }
    ...
    for($y=1;$y<=$skaiciusM;$y++){
        $skaicius=0;

        $sql = "SELECT ID, Vardas, Pavarde, MokinioEL, Slaptazodis, Statusas FROM Mokiniai WHERE MokytojoID='$mokytojoID[$y]'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            $skaicius++;
        ...
        // Add a recipient.
        $email->addAddress($pastas[$y], "$vardas[$y] $pavarde[$y]");

        // Set the subject.
        $email->Subject = "Mėnesinė $klasesNumeris[$y] klasės/srauto ataskaita";
                                    ...
                                    <table border='0' cellpadding='0' cellspacing='0' width='100%'>
                                        <tr>
                                            <td style='color: #153643; font-family: Arial, sans-serif; font-size: 15px;'>
                                                <b>Sistema suformavo kas mėnesinę Jūsų vadovaujamos $klasesNumeris[$y] klasės/srauto aktyvumo ataskaitą platformoje „VDU UKG Žinių mainai“</b>
                                            </td>
                                        </tr>
